...
## 1. テキストは最小限、図解・ビジュアルが主役
- **1 スライド 1 メッセージ**。詰め込み禁止。
- 本文（highlight_box / outro）は **40〜70 字** を目安。120 字超は即 NG。
- 情報量が多いなら、**スライドを分割**してでも 1 スライドの文字量を減らす。
- 写真や図解だけのスライドは NG（必ず上か下にテキスト）だが、逆に**テキスト過多も NG**。
...
## 5. 読者の行動変容を促す
- スライド末尾（outro）は**次のスライドへの誘導**か**行動指南**。
- キャプションは**保存・フォロー**のアクションに自然につなげる。

## 6. 専門用語は噛み砕く
- 金融・不動産用語は初出で**補足**（例：「代償分割（〇〇を現金で分ける方法）」）。
- 難しい語は**より平易な語**に置き換え（ただし企業発信としての品位は保つ）。
...
- zone.content は**最大 70 字**。長い説明は次スライドへ分割。
