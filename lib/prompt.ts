...
**同じ内容（主旨・ターゲット・テーマ）を、相応しい範囲で見せ方や言い回しを
アレンジした 3 パターン**を出すこと。案ごとに別のネタに見えてはいけない。

### アレンジの方向性（3 案で意図的に変える観点）

下記から **3 つの異なる切り口**を選んで使い分ける（金融機関の公式発信として
不適切にならない範囲で）：

- **ベネフィット訴求**：得られる結果・読後の効用を前面に
  例：『〇〇で賢く選ぶ方法』『知って差がつく〇〇』
- **問いかけ・呼びかけ**：読者に立ち止まらせる
  例：『〇〇、ご存じですか？』『〇〇で迷っていませんか？』
- **中立解説**：客観的・教科書的
  例：『〇〇の基礎と選び方』『〇〇とは？仕組みから解説』
- **注意喚起（穏やか）**：見落とし・留意点を提示
  例：『〇〇で見落としがちなポイント』『〇〇の前に確認したいこと』
- **数字・具体性訴求**：数字や具体ワードで明確化
  例：『〇〇 5 つのポイント』『〇〇を整理する〇つの観点』

### 守るべきルール

- 3 案は**同じテーマを指す**こと（中身が分散しない）。
- 各案 20〜28 字程度。数字は半角。
- 口語・造語（『即アウト』『マジで』等）は禁止。
- 過度に煽る表現（『絶対』『今すぐ』『損する』等の連呼）は避ける。
- 投稿フォーマットの title_templates を参考にしつつ、上記の切り口で言い換える。
...
{
  "titles": [
    {"approach": "ベネフィット訴求", "text": "〇〇で賢く選ぶ方法"},
    {"approach": "中立解説", "text": "〇〇の基礎と選び方"},
    {"approach": "注意喚起", "text": "〇〇で見落としがちなポイント"}
  ],
  "slides": [
    {
      "index": 1,
